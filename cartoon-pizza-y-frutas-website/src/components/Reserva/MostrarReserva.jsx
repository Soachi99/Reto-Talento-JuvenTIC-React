...
export default function MostrarReserva() {
  ...
  useEffect(() => {
    fetch("https://talento-juventic-backend.herokuapp.com/api/reserva")
      .then((res) => res.json())
      .then((data) => {
        setReserva(data);
        setCarga(false);
        ...
        <div className="contenedor-reserva">
          {reserva.map((r) => (
            <div className="reserva" key={r.id}>
              ...
              <p>
                <span>Indicacion especial: </span>
                {r.indicacion_especial}
              </p>
              <a
                className="mt-2 p-2 btn btn-danger"
                href={`https://talento-juventic-backend.herokuapp.com/api/reserva/${r.id}`}
              >
                Eliminar
              </a>
            </div>
